...
static void compareAll(const std::string& orbAll, const std::string& gtsamAll, double absTol, double relTol) {
  ...
  const Eigen::MatrixXd dR_orb = getBlockOrThrow(orb, "dR_orb", orbAll);
  const Eigen::MatrixXd dR_gtsam = getBlockOrThrow(gtsam, "dR_gtsam", gtsamAll);
  const Eigen::MatrixXd dP_orb = getBlockOrThrow(orb, "dP_orb", orbAll);
  const Eigen::MatrixXd dP_gtsam = getBlockOrThrow(gtsam, "dP_gtsam", gtsamAll);
  const Eigen::MatrixXd dV_orb = getBlockOrThrow(orb, "dV_orb", orbAll);
  const Eigen::MatrixXd dV_gtsam = getBlockOrThrow(gtsam, "dV_gtsam", gtsamAll);
  const Eigen::MatrixXd DT_orb = getBlockOrThrow(orb, "DT_orb", orbAll);
  ...
  ok &= expectNearAbsRel(dP_orb, dP_gtsam, absTol, relTol, "dP");
  ok &= expectNearAbsRel(dV_orb, dV_gtsam, absTol, relTol, "dV");
  ok &= expectNearAbsRel(DT_orb, DT_gtsam, absTol, relTol, "DT");
