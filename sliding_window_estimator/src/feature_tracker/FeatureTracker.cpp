...
void FeatureTracker::stereoCallback(
  ...
  cam0_curr_img_ = cam0_img;
  cam0_curr_img_header_ = cam0_header;
  cam1_curr_img_ = cam1_img;
...
void FeatureTracker::prepareForNextFrame() {
  ...
  cam0_prev_img_ = cam0_curr_img_;
  cam0_prev_img_header_ = cam0_curr_img_header_;
...
void FeatureTracker::createImagePyramids() {
  const Mat& curr_cam0_img = cam0_curr_img_;
  ...
  if (!processor_config.monocular) {
    const Mat& curr_cam1_img = cam1_curr_img_;
    buildOpticalFlowPyramid(
...
void FeatureTracker::initializeFirstFrame() {
  ...
  for (auto& item : grid_new_features)
    std::sort(item.second.begin(), item.second.end(),
        &FeatureTracker::featureCompareByResponse);
...
void FeatureTracker::trackFeatures() {
  ...
  static int grid_height =
    cam0_curr_img_.rows / processor_config.grid_row;
  static int grid_width =
    cam0_curr_img_.cols / processor_config.grid_col;
  ...
  for (int i = 0; i < static_cast<int>(curr_cam0_points.size()); ++i) {
    ...
    if (curr_cam0_points[i].y < 0 ||
        ...
        curr_cam0_points[i].x > cam0_curr_img_.cols-1)
      track_inliers[i] = 0;
  }
...
void FeatureTracker::stereoMatch(
  ...
  for (int i = 0; i < static_cast<int>(cam1_points.size()); ++i) {
    ...
    if (cam1_points[i].y < 0 ||
        ...
        cam1_points[i].x < 0 ||
        cam1_points[i].x > cam1_curr_img_.cols-1)
      inlier_markers[i] = 0;
  }
...
void FeatureTracker::addNewFeatures() {
  const Mat& curr_img = cam0_curr_img_;
...
void FeatureTracker::pruneGridFeatures() {
...
void FeatureTracker::integrateImuData(
  ...
  while (begin_iter != imu_msg_buffer.end()) {
    if ((begin_iter->header.stamp-
          cam0_prev_img_header_.stamp).toSec() < -0.01)
      ++begin_iter;
    else
      break;
  }
  ...
  while (end_iter != imu_msg_buffer.end()) {
    if ((end_iter->header.stamp-
          cam0_curr_img_header_.stamp).toSec() < 0.005)
      ++end_iter;
    else
      break;
  }
  ...
  double dtime = (cam0_curr_img_header_.stamp -
      cam0_prev_img_header_.stamp).toSec();
...
inline cv::Scalar featureColor(int life_time) {
  int WINDOW_SIZE=20;
  double len = std::min(1.0, 1.0 * life_time / WINDOW_SIZE);
  return cv::Scalar(255 * (1 - len), 0, 255 * len);
}

cv::Mat FeatureTracker::drawFeaturesMono() {
  ...
  int img_height = cam0_curr_img_.rows;
  int img_width = cam0_curr_img_.cols;
  Mat out_img(img_height, img_width, CV_8UC3);
  cvtColor(cam0_curr_img_, out_img, cv::COLOR_GRAY2RGB);
  ...
  for (int i = 1; i < processor_config.grid_row; ++i) {
    ...
    line(out_img, pt1, pt2, Scalar(255, 0, 0), grid_thickness);
  }
  ...
  map<FeatureIDType, std::pair<Point2f, int>> curr_points;
  for (const auto& grid_features : *curr_features_ptr)
    for (const auto& feature : grid_features.second)
      curr_points[feature.id] = std::make_pair(
          feature.cam0_point, feature.lifetime);
  ...
  for (const auto& id : prev_ids) {
    if (prev_points.find(id) != prev_points.end() &&
      ...
      cv::Point2f curr_pt = curr_points[id].first;
      cv::Scalar tracked_color = featureColor(curr_points[id].second);
      circle(out_img, curr_pt, 3, tracked_color, circle_thickness);
      line(out_img, prev_pt, curr_pt, tracked_color, line_thickness);
  ...
  for (const auto& new_curr_point : curr_points) {
    cv::Point2f pt = new_curr_point.second.first;
...
cv::Mat FeatureTracker::drawFeaturesStereo() {
  // Colors for different features.
  Scalar tracked(0, 255, 0);
  Scalar new_feature(0, 255, 255);
  ...
  cvtColor(cam0_curr_img_,
            out_img.colRange(0, img_width), cv::COLOR_GRAY2RGB);
  cvtColor(cam1_curr_img_,
            out_img.colRange(img_width, img_width*2), cv::COLOR_GRAY2RGB);
...
void FeatureTracker::updateFeatureLifetime() {
  for (int code = 0; code <
      processor_config.grid_row*processor_config.grid_col; ++code) {
...
void FeatureTracker::featureLifetimeStatistics() {
