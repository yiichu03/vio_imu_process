...
template <int N> Eigen::Matrix<double, N, N> randomCovariance() {
...
void assertEqual(const M1 &A, const M2 &B, okvis::source_file_pos const &sfp,
...
void assertNear(const M1 &A, const M2 &B, T tolerance,
                okvis::source_file_pos const &sfp,
                std::string const &message = "") {
...
void assertFinite(const M1 &A, okvis::source_file_pos const &sfp,
                  std::string const &message = "") {
  for (int r = 0; r < A.rows(); r++) {
    for (int c = 0; c < A.cols(); c++) {
...
#define ASSERT_DOUBLE_MX_EQ(A, B, PERCENT_TOLERANCE, MSG)                      \
  ...
  for (int r = 0; r < (A).rows(); r++) {                                       \
    for (int c = 0; c < (A).cols(); c++) {                                     \
      ...
      ASSERT_TRUE(eigen::compareRelative(                                  \
...
void printTwoMatrices(const Eigen::MatrixBase<Derived1> &mat1,
                      const Eigen::MatrixBase<Derived2> &mat2,
                      const std::vector<std::string> descriptions, int row,
                      int col, int rowsize, int colsize) {
  LOG(INFO) << descriptions[0] << "(" << row << ", " << col << ", " << rowsize
            << ", " << colsize << ")\n"
            << mat1.block(row, col, rowsize, colsize) << "\n"
            << descriptions[1] << "(" << row << ", " << col << ", " << rowsize
            << ", " << colsize << ")\n"
            << mat2.block(row, col, rowsize, colsize) << "\n"
            << "diff\n"
            << mat1.block(row, col, rowsize, colsize) -
                   mat2.block(row, col, rowsize, colsize);
}
