...
if __name__ == '__main__':
    ...
    for name, options in algoname_to_options.items():
        for bag_index, bag_fullname in enumerate(bag_list):
            ...
            data_fullname = os.path.splitext(bag_fullname)[0]
            ...
            for trial_index in range(args.num_trials):
                ...
                if dataset == "euroc":
                    calib_path = "/home/youkely/dev/basalt-mirror/data/euroc_ds_calib.json"
                    config_path = "/home/youkely/dev/basalt-mirror/data/euroc_config.json"
                    cmd = "~/dev/basalt-mirror/build/basalt_vio --dataset-path {} --cam-calib {} --dataset-type euroc --config-path {} --save-trajectory tum --show-gui 0".format(
                        data_fullname, calib_path, config_path)
                    eval_cfg_template = os.path.join(args.catkin_ws, "src/msckf/evaluation/config/eval_cfg.yaml")


                elif dataset == "tumvi":
                    calib_path = "/home/youkely/dev/basalt-mirror/data/tumvi_512_eucm_calib.json"
