> OUTDATED（2026-09-08）：历史需求原文和开发对话汇编，含有限差分替换、旧接口等已废弃方案；原始需求仍有追溯价值，但其中的实现/结果描述不代表当前版本。当前说明见 [验证入口](imu_data/README.md) 和工作区 [交接总览](../PROJECT_HANDOVER.md)。

# VINS MONO
需要修改一下代码，把不使用有限差分法的结果也保存下来，运行看一下能差多少/
代码逻辑：

1. 先用 VINS 的IntegrationBase对 IMU 数据做预积分，得到 VINS 原生的预积分增量（ΔR/Δp/Δv）、雅可比（jac'）和协方差（cov'）；
2. 构建了误差状态转换矩阵 A，这个 A 是 GTSAM 误差状态相对于 VINS 误差状态的雅可比，主要解决了两个差异：一是 VINS 和 GTSAM 的误差维度顺序不同（GTSAM 把旋转李代数放第一位），二是旋转误差的定义不同（GTSAM 是李代数加法扰动，VINS 是角轴右乘扰动），同时翻转了偏置残差的符号以匹配 GTSAM 的定义；
3. 用转换矩阵 A 将 VINS 的 cov' 转换为 GTSAM 格式的协方差；
4. 对于旋转增量对陀螺偏置的雅可比，用有限差分法验证并修正了解析解，确保和 GTSAM 的误差定义完全一致，其他偏置雅可比直接复用 VINS 的解析解（因为定义无差异）；

最终得到了符合 GTSAM 误差状态定义的预积分因子的雅可比（尤其是偏置雅可比）和协方差。
...
```
我觉得我首先应该把老师的项目环境编译好，首先想能跑通，你觉得你可以根据readme指导我如何配置环境（比如用docker）吗？然后写一个代码读取imu数据、参考propag_by_preint代码写一个新的代码（但尽量小幅度或者不修改其他代码），用来处理imu数据得到一个输出（随便什么输出，可以是对我们之后有用的输出）验证这个项目正确编译了。
```
后续步骤（先对齐思路）

第 1 步：把老师项目 swift_vio 的 catkin 工作空间编译通过（含 GTSAM + GTSAM_UNSTABLE），先用项目自带的 propag_by_preint 做“零改动冒烟测试”验证环境没问题（它已在 CMakeLists.txt (line 296) 注册为可执行程序）。
第 2 步：再新增一个“最小 app”（只加 1 个 .cpp + 2 行 CMake），读取 *.txt，做一次简单预积分/统计输出，用来验证“能读你的 IMU 文件 + 能链接到核心库/GTSAM”。
第 3 步：以上都 OK 后，再进入你真正要做的路线A：把 OpenVINS propagation 导出的中间量对齐/映射到你老师工程的 gtsam tangent preint（这步我们后面再做）。
1) 环境配置（推荐 Docker，可复现）
老师 README 里没有现成 Docker，但依赖写得很清楚（README.md (line 20) 起）。我建议用 Ubuntu 20.04 + ROS noetic（和你 OpenVINS 环境一致），并源码编译 GTSAM（开启 unstable）：

启动一个干净的 Noetic 容器

docker run --rm -it \
  -v "$PWD/JzHuai0108-swift_vio-2aa4b7d13701:/ws/src/swift_vio:ro" \
...
apt-get update && apt-get install -y \
  ...
  libtbb-dev libceres-dev
rosdep init || true
rosdep update
容器内编译 GTSAM（按 README 固定 commit，且开启 unstable）
...
make -j"$(nproc)"
make install
关键注意：依赖仓库拉取方式（SSH vs HTTPS）

你的 dependencies.rosinstall 里有 git@github.com: 和 git@bitbucket.org:（例如 dependencies.rosinstall (line 9)）。在 Docker 里如果没有 SSH key 会拉不下来。
两个选项：
A）把 ~/.ssh 挂进容器并确保权限（适合私有 repo）
B）把 rosinstall 里的 git@... 替换成 https://...（适合公开 repo）
2) “零改动冒烟测试”：直接跑 propag_by_preint
你现在的仓库已经自带可执行程序 propag_by_preint（CMakeLists.txt (line 296)），这是最适合“确认编译/链接完全正确”的验证。

典型流程（假设你在容器里另建工作空间并把依赖拉齐后）：

# 例：/ws 是 catkin workspace
source /opt/ros/noetic/setup.bash
cd /ws
catkin build sliding_window_estimator --cmake-args -DGTSAM_DIR=/usr/local/lib/cmake/GTSAM -DGTSAM_UNSTABLE_DIR=/usr/local/lib/cmake/GTSAM_UNSTABLE
source /ws/devel/setup.bash

rosrun sliding_window_estimator propag_by_preint
只要它能正常运行并输出对比结果/不崩溃，你的核心依赖链路（gtsam/okvis/ceres 等）基本就通了。

3) 新增一个最小程序：读 *.txt 并输出（用于“能读你的数据”验证）
最小改动方案：

新增文件：imu_file_smoketest.cpp
CMake 加两行（紧挨着 propag_by_preint 那段即可）：CMakeLists.txt (line 296)
imu_file_smoketest.cpp（示例：读取并做一次 GTSAM 预积分后打印）
...
CMake 增量（加在 propag_by_preint 下面）
...
rosrun sliding_window_estimator imu_file_smoketest --imu_txt=/path/to/imu_data/imu_data_Tangent_0.txt
如果你愿意，我下一步可以按你的 cpc_config_*.yaml 把 bias/noise/gravity 自动读出来（避免手填 flag），但这会多引入一个 YAML 解析依赖/写几行解析代码。
